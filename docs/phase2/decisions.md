...
| **C5** | Observability | **OpenTelemetry tracing + Micrometer/Prometheus metrics** (R15), designed to run per-claim/per-stage in `claims-service` and `rxclaim-emulator` | ✅ **Accepted** | Design documented ([`plan.md` §5](./plan.md#5-cloud-architecture--compute-data-security-observability-scalability)); found undelivered post-hoc (2026-08 — the milestone table's "OTel tracing wired" / "Managed-Prometheus metric names" touchpoints were aspirational, not built). **Closed by a later platform-wide observability effort**: both services now carry real `opentelemetry`/`micrometer-tracing` dependencies, trace-context propagation on every outbound call, and an exposed `/actuator/prometheus` endpoint — see that effort's own docs for the full design, not restated here to avoid this entry going stale if that effort's own internal numbering ever changes. |
